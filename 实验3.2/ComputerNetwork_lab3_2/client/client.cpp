...
DWORD WINAPI RecvThread(LPVOID lparam) {
    ...
    while (true) {
        while (!timesaver.empty()) //时间队列有东西
        {
            ...
            if (checksum(recv, 8) == 0 && recv[2] == ACK) {
                if (seq == SendBase+1) {
                    // 收到了正确的报文，窗口向后面移动一下
                    addSampleRTT(sampleRTT);
                    timesaver.pop();
...
void GBN_Send() {
    ...
    while (true)
    {
        while (curSeq-SendBase<(window_size))
        {
            int last = curSeq + window_size ;
            if (last > lastSeq)
                last = lastSeq;
